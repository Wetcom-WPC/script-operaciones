...
function getClientConfigByName(clientName, operationName) {
  try {
    ...
    const clientRow = masterData.find(row => row && row[1] != null && String(row[1]).trim().toLowerCase() === clientName.toLowerCase());
...
function buscarYCerrarTareaProgramada(taskNameBase, clientConfig, useClientNameInTask, isAVS = false) {
  if (!taskNameBase || !clientConfig || !clientConfig.clientName) {
    return { status: 'SKIPPED' };
  }

  if (isAVS) {
    taskNameBase = "AVS - " + taskNameBase;
  }
...
function construirBusquedaGmail(emailSubject) {
  ...
  // Filtra las celdas vacías y extrae los remitentes (soporta múltiples separados por coma)
  const senders = [];
  senderData.flat().forEach(cell => {
    if (cell && cell.trim() !== "") {
      cell.split(',').forEach(s => {
        const cleanSender = s.trim();
        if (cleanSender !== "" && senders.indexOf(cleanSender) === -1) {
...
function getClientConfig(senderEmail, operationName, soporte = false) {
  try {
    ...
    const clientRow = masterData.find(row => {
      ...
      const senders = cellValue.split(',').map(s => s.trim().toLowerCase());
      return senders.includes(domain.toLowerCase());
    });
...
function findExistingJiraTicket(summary, projectKey, issueTypeName) {
  ...
  let jql = `summary ~ "${safeSummary}" AND statusCategory != "Done"`;
  if (projectKey) jql += ` AND project = "${projectKey}"`;
  if (issueTypeName) jql += ` AND issuetype = "${issueTypeName}"`;
  jql += " ORDER BY created DESC";
  Logger.log(`[JQL DEBUG] Ejecutando query en Jira: ${jql}`);
