...
function processSingleOrphanedVMsMessage(message, summaryReport) {
  ...
  if (!attachmentToUse) {
    ...
    if (!isComafi) {
      const closeResult = buscarYCerrarTareaProgramada(ORPHANED_VMS_TASK_NAME, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
