...
function processSingleInaccessibleVMMessage(message, summaryReport) {
  ...
  if (finalAlerts.length === 0) {
    ...
    const closeResult = buscarYCerrarTareaProgramada(INACCESSIBLE_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false);    if(closeResult && closeResult.status === 'SUCCESS') summaryReport.tareasCerradas++;
  ...
  if (existingTicketKey) {
    ...
    if (attachmentStatus.status === 'SUCCESS') {
      const closeResult = buscarYCerrarTareaProgramada(INACCESSIBLE_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false);
