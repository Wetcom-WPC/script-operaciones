...
function processSingleUndersizedVMsMessage(message, summaryReport) {
  ...
  if (existingTicketKey) {
    ...
    if (attachmentResult.status === 'SUCCESS') {
      ...
      const closeResult = buscarYCerrarTareaProgramada(UNDERSIZED_VMS_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
