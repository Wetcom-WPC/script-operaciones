 ...
 * ARCHIVO UNIFICADO: Motor + Front-End (Atado a Planilla)
 * =================================================================
...
// 5. Función de Procesamiento Aislado
...
function ejecutarClienteSeleccionado() {
  ...
  // Validar que estemos en la pestaña correcta
  if (hoja.getName() !== NOMBRE_PESTANA_CONFIG) {
    ui.alert("⚠️ Advertencia", `Debes estar parado en la pestaña '${NOMBRE_PESTANA_CONFIG}' para ejecutar esta acción.`, ui.ButtonSet.OK);
    return;
  }
  ...
  if (fila === 1) {
    ui.alert("⚠️ Advertencia", "Selecciona una fila de cliente válida (Fila 2 en adelante).", ui.ButtonSet.OK);
    return;
  }
  ...
  try {
    // Al estar unificado, llamamos a la función local directamente.
    const respuestaMotor = procesarLicenciasManualLibreria(cliente, emailDestino, folderId, pod);
    
    if (respuestaMotor.success) {
      ui.alert("✅ Éxito", respuestaMotor.message, ui.ButtonSet.OK);
    } else {
      ui.alert("❌ Error en Motor", respuestaMotor.message, ui.ButtonSet.OK);
    }
  } catch (error) {
    ui.alert("❌ Error de Conexión", `Hubo un fallo inesperado: ${error.message}`, ui.ButtonSet.OK);
  } finally {
...
/**
 * =================================================================
 * 3. MOTOR PRINCIPAL Y CONTROL DE TIEMPO
 * =================================================================
 */
...
/**
 * =================================================================
 * 4. HERRAMIENTAS DE PROCESAMIENTO Y CALENDARIO
 * =================================================================
 */
...
function esUltimoDiaHabilMes() {
  ...
  let feriadosDelMes = [];
  try {
    const calendarId = 'es.ar#holiday@group.v.calendar.google.com'; 
    ...
    console.warn("⚠️ No se pudo acceder al calendario de feriados. Usando fallback.", e.message);
  }
  ...
  for (let dia = ultimoDiaDelMes; dia > 0; dia--) {
    ...
    const diaSemana = fechaTest.getDay(); 
...
/**
 * =================================================================
 * 5. SISTEMA DE ALERTAS (REPORTING MULTI-ESTADO COMPLETO)
 * =================================================================
 */
