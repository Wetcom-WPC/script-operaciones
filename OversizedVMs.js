...
// --- CONFIGURACIÓN ESPECÍFICA DE Oversized VMS ---
const OVER_VMS_OPERATION_NAME = "Oversized VMs";
const OVER_VMS_EMAIL_SUBJECT = "Oversized VMs"; // CONFIRMAR: Ajusta el asunto del email si es diferente
const OVER_VMS_CSV_FILENAME_MATCH = "Oversized VMs";    // CONFIRMAR: Ajusta el nombre del archivo si es diferente
const OVER_VMS_SCHEDULED_TASK_NAME_TO_CLOSE = "Oversized VMs";
const OVER_VMS_JIRA_TICKET_SUMMARY = "Se detectaron Oversized VMs";


// --- LÓGICA PRINCIPAL DE Oversized VMS ---
...
function processOversizedVMsEmails() {
  ...
  const searchQuery = construirBusquedaGmail(OVER_VMS_EMAIL_SUBJECT);
  ...
  if (threads.length > 0) {
    threads.forEach(thread => {
      ...
      if (message.isUnread()) {
        try {
          const result = processSingleOversizedVMsMessage(message);
  ...
  enviarResumenSlack(OVER_VMS_OPERATION_NAME, summaryReport);
}


function processSingleOversizedVMsMessage(message) {
  const senderEmail = message.getFrom();
  const attachment = message.getAttachments().find(att =>
    att.getName().includes(OVER_VMS_CSV_FILENAME_MATCH) && att.getContentType().includes("text/csv")
  );

  if (!attachment) return { status: 'NO_OP' };

  const clientConfig = getClientConfig(senderEmail, OVER_VMS_OPERATION_NAME);
  if (!clientConfig) {
  ...
  const existingTicketKey = findExistingJiraTicket(OVER_VMS_JIRA_TICKET_SUMMARY, clientConfig.jiraProjectKey);
  ...
  if (finalAlerts.length === 0) {
    if (existingTicketKey) {
      ...
      const closeResult = buscarYCerrarTareaProgramada(OVER_VMS_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
  ...
  if (existingTicketKey) {
    ...
    if (attachmentResult.status === 'SUCCESS') {
      ...
      const closeResult = buscarYCerrarTareaProgramada(OVER_VMS_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
      const accountIdAsignado = chequearSiEsInformativa(clientConfig.clientName, OVER_VMS_JIRA_TICKET_SUMMARY); 
    ...
    const creationResult = analyzeOversizedVMs_CSV(attachment.getName(), originalHeaders, finalAlerts, clientConfig);
...
function analyzeOversizedVMs_CSV(attachmentName, headers, finalAlerts, clientConfig) {
  const alertCount = finalAlerts.length;
  const summary = OVER_VMS_JIRA_TICKET_SUMMARY;
  ...
  return createTicketAndNotify(summary, description, xlsxBlob, clientConfig,OVER_VMS_OPERATION_NAME);
}