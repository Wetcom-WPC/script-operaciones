...
function processSingleVeeamJobMessage(message, summaryReport) {
  ...
  if (emailSubject.toLowerCase().includes("success")) {
    ...
    const closeResult = buscarYCerrarTareaProgramada(VEEAM_SCHEDULED_TASK_NAME, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
