            ...
            if (clienteInfo.projectKey) {
              for (var k = 0; k < REPORTES_QUE_CIERRAN_TAREAS_LOWER.length; k++) {
                if (nombreArchivoLower.includes(REPORTES_QUE_CIERRAN_TAREAS_LOWER[k])) {
                  ...
                  var isAVS = (nombreArchivoLower.includes('avs') || (mensaje && mensaje.getSubject().toLowerCase().includes('avs')));
                  var taskNameToSearch = REPORTES_QUE_CIERRAN_TAREAS_BASE[k];
                  if (isAVS) {
                    taskNameToSearch = "AVS - " + taskNameToSearch;
                  }
                  var tareaKey = findExistingJiraTicket(taskNameToSearch, clienteInfo.projectKey, "Tarea Programada");
