...
function processSingleVropsMessage(message, summaryReport) {
  ...
  if (!allRows || allRows.length < 2) {
    ...
    const closeResult = buscarYCerrarTareaProgramada(VROPS_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
