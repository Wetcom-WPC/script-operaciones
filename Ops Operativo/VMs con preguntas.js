...
function processSingleVmWithQuestionMessage(message, summaryReport) {
  ...
  if (!reportData || !Array.isArray(reportData) || reportData.length === 0) {
    const closeResult = buscarYCerrarTareaProgramada(Q_VM_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
