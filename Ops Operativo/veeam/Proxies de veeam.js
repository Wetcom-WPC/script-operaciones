...
class ProxiesVeeamProcessor extends MailProcessor {
  ...
  processEmails() {
    ...
    for (const sender in alertsByClient) {
      if (!timeGuard.check(`Remitente: ${sender}`)) {
        summaryReport.advertencias.push({
          ticketKey: "N/A",
          problema: "Límite de tiempo de ejecución alcanzado o margen <30s (TimeGuard).",
          accion: `Se pausó el procesamiento de alertas de proxy por seguridad. Quedan alertas pendientes de ${sender} para la próxima ejecución.`
        });
        break;
      }
