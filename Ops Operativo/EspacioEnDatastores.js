...
function processSingleDatastoreMessage(message, summaryReport) {
  ...
  if (finalAlerts.length === 0) {
    ...
    const closeResult = buscarYCerrarTareaProgramada(DS_SCHEDULED_TASK_NAME_TO_CLOSE, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
