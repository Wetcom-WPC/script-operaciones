...
function gestionarReporteRVTools(clientConfig, summaryReport, taskName, ticketTitle, headers, allAnomalies) {
  try {
    ...
    const closeResult = buscarYCerrarTareaProgramada(taskName, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
