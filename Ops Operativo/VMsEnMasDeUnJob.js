...
function processSingleDuplicateJobMessage(message, summaryReport) {
  ...
  if (!attachmentToUse) {
    ...
    const closeResult = buscarYCerrarTareaProgramada(VMS_EN_MAS_DE_UN_JOB_TASK_NAME, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
