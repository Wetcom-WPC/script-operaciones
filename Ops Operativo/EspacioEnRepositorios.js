...
function processSingleRepositorySpaceMessage(message, summaryReport) {
  ...
  if (alertCount === 0) {
    ...
    const closeResult = buscarYCerrarTareaProgramada(REPO_SPACE_TASK_NAME, clientConfig, false, (typeof message !== 'undefined' && message) ? (message.getSubject().toLowerCase().includes('avs') || (message.getAttachments && message.getAttachments().some(a => a.getName().toLowerCase().includes('avs')))) : false);
